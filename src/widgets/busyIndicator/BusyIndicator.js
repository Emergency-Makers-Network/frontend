...
export default function BusyIndicator({ children }) {
    ...
    const hasContentToDisplay = !show && children && (children.length === undefined || children.length > 0);
    ...
    return (
        <React.Fragment>
            {show ? (
                ...
                <React.Fragment>{hasContentToDisplay ? children : <ContentNotFound />}</React.Fragment>
            )}
        </React.Fragment>
    );
}
