...
import { REQUEST_ALREADY_PENDING_ASYNC, TURN_OFF_BUSY_INDICATOR_FOR_PENDING_ASYNC } from './doAsync.actionTypes';
import { addPendingRequest, deletePendingRequest, setBusySpinner, selectPendingRequest } from '../pendingRequest';
import { notifyError } from '../notificationPopup';
import { decrementBusyIndicator, incrementBusyIndicator } from '../../widgets/busyIndicator';

export function cleanUpPendingRequests({ url, httpMethod, busyIndicatorName, dispatch, getState }) {
...
export function handleError(exception, onError, dispatch, httpMethod, url, httpConfig, errorMessage) {
    if (onError) {
        onError(exception);
    } else {
...
export function logError(dispatch, httpMethod, url, httpConfig, { exception, errorMessage } = {}) {
    console.log(
...
export function validateInput(url, httpMethod) {
    ...
    if (!httpMethod || !http[httpMethod]) {
        throw new Error('httpMethod is required and must index the http service and resolve to a method.');
    }
}
