...
import { incrementBusyIndicator, decrementBusyIndicator } from '../../widgets/busyIndicator';
...
const doAsync = ({
    ...
    if (!getState || typeof getState !== 'function') {
        throw new Error('getState is required and must have a getState method defined on it');
    }
    ...
    try {
        ...
        if (useCaching) {
            if (tryToFindRequestInCache(getState(), url, httpMethod, httpConfig && httpConfig.body)) {
        ...
        return http[httpMethod](url, httpConfig, { stubSuccess, stubError })
            ...
            .catch((exception) => {
                handleError(exception, onError, dispatch, httpMethod, url, httpConfig, errorMessage);
            })
            .then((response) => {
                cleanUpPendingRequests({
                    url,
