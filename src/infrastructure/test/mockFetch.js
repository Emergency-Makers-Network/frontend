...
const mockFetch = (stubConfig, { calls } = {}) => {
            ...
            return this.find(
                (c) =>
                    ((!c.config.method && (!method || method.toLowerCase() === 'get')) ||
                        (c.config.method && c.config.method.toLowerCase() === method.toLowerCase())) &&
                    c.url.includes(url),
            );
        };
    ...
    return (url, config) => {
        ...
        const stubMethodMatchesConfig = (stubUrlConfig) =>
            stubUrlConfig.method && config.method && config.method.toLowerCase() === stubUrlConfig.method.toLowerCase();

        const methodIsGetOrMethodsMatch = (stubUrlConfig, curConfig) =>
            isGetOrDefault(stubUrlConfig, curConfig) || stubMethodMatchesConfig(stubUrlConfig);
        ...
        const foundUrl = stubConfig.find(
            (stubUrlConfig) =>
                methodIsGetOrMethodsMatch(stubUrlConfig, config) && urlIncludesFragments(url, stubUrlConfig.url),
        );
