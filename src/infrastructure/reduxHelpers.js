...
export const buildAsyncActionType = (module, actionType) => {
    ...
    return asyncStates.reduce((accumulator, asyncState) => {
        accumulator[asyncState] = `${buildActionType(module, actionType)}_${asyncState}_ASYNC`;
        return accumulator;
    }, {});
};

export const buildActionType = (module, actionType) => `${ACTION_TYPE_PREFIX}/${module}/${actionType}`;

export const mergeCollections = (elements, newElements, idProperty = 'id', mergeType = mergeTypes.INTERSECT) => {
...
const intersectCollections = (elements, newElements, idProperty) => {
    ...
    return [...elements, ...newElements].reduce((acc, cur) => {
        ...
        if (foundIndex === NOT_FOUND) {
            if (newElements.map((newEl) => newEl[idProperty]).includes(cur[idProperty])) {
                acc.push(cur);
            }
            return acc;
        }
